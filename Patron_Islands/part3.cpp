...
int main() {
  ...
  long long out = 0;
  out += get_manhattan(sorted_islands[0]);

  for (int i = 1; i < sorted_islands.size(); i++) {
    WorldSpace::set_position(
        std::make_tuple(std::get<0>(sorted_islands[i - 1]),
                        std::get<1>(sorted_islands[i - 1])));

    std::sort(sorted_islands.begin() + i, sorted_islands.end(), sort_islands);

    out += get_manhattan(sorted_islands[i]);
  }
